...
const LLMs = [
  {
    emoji: "  ", name: "Grok 1.5", provider: "xAI",
...
const DATA = {
  papersCore: [
    {
      t: "Codd: A Relational Model for Large Shared Data Banks",
      url: "https://dl.acm.org/doi/10.1145/362384.362685",
      year: 1970,
      summary: "Introduced the relational model for database management."
    },
    {
      t: "Dynamo",
      url: "https://www.allthingsdistributed.com/2007/10/amazons_dynamo.html",
      year: 2007,
      summary: "Amazon's highly available key-value store for large-scale services."
    },
    {
      t: "Other",
      url: "https://github.com/papers-we-love/papers-we-love",
      year: "",
      summary: "More essential database and systems papers worth exploring."
    },
    {
      t: "Scaling Memcache at Facebook",
      url: "https://research.facebook.com/publications/scaling-memcache-at-facebook/",
      year: 2013,
      summary: "How Facebook scales ephemeral caching across global regions."
    },
    {
      t: "Spanner",
      url: "https://research.google/pubs/pub41344/",
      year: 2012,
      summary: "Planet-scale, strongly consistent distributed SQL from Google."
    },
    {
      t: "The End of an Architectural Era",
      url: "https://db.csail.mit.edu/pubs/stonebraker-cacm07.pdf",
      year: 2007,
      summary: "Why traditional RDBMS designs struggled with modern analytics workloads."
    },
    {
      t: "The NoSQL Movement",
      url: "https://cacm.acm.org/magazines/2012/6/149798-the-nosql-movement/fulltext",
      year: 2012,
      summary: "Examines how scale pressures led teams to schema-light data stores."
    }
  ],
  paperGroups: [
    {
...
const CRYPTOS = [
  ...
  { emoji:"  ", symbol:"PYUSD", name:"PayPal USD", type:"Stablecoin",
    network:"Ethereum",
...
/* Render all papers in alphabetical order */
function renderPapers() {
  const ul = $("#papers-list");
  if (!ul) return;
  
  // Collect all papers into a flat list
  const allPapers = [
    ...DATA.papersCore.map(p => ({ 
      title: p.t, 
      url: p.url, 
      year: p.year, 
      summary: p.summary 
    })),
    ...DATA.paperGroups.flatMap(group => 
      group.items.map(p => ({
        title: p.t,
        url: p.url,
        summary: p.note
  ...
  uniquePapers.sort((a, b) => a.title.localeCompare(b.title));
...
(() => {
  ...
  const $ = (sel, ctx=document) => ctx.querySelector(sel);
  const grid = $('#pg-grid');
  const controlsWrap = $('#pg-controls');
  const controlsToggle = $('#pg-controls-toggle');
  const controlsPanel = $('#pg-controls-panel');
  const searchEl = $('#pg-search');
  const sortEl = $('#pg-sort');
  const searchBtn = $('#pg-search-btn');
  const statusWrap = $('#pg-status-wrap');
  const resultsWrap = $('#pg-results');
  const statusToggle = $('#pg-status-toggle');
  const statusLabel = statusToggle ? statusToggle.querySelector('.pg-status__label') : null;
  ...
  const dictEls = {
    root: $('#pg-dictionary'),
    filters: $('#pg-dict-filters'),
    actionSeg: $('#pg-dict-actions'),
    styleSeg: $('#pg-dict-styles'),
    feedback: $('#pg-dict-feedback'),
    printBtn: $('#pg-dict-print'),
    printable: $('#pg-dict-printable')
  };
  const dictState = {
    type: 'action',
    action: '',
  ...
  if (!grid || !searchEl || !sortEl || !searchBtn || !schemaEl) return;
  ...
  function uniqueSorted(arr) {
    return Array.from(new Set(arr.filter((item) => item != null && item !== '')))
  ...
  function update() {
    ...
    if (!statusWrap || statusWrap.dataset.open !== 'true') {
      setStatusOpen(false);
    }
  ...
  function initDictionary() {
    if (!dictEls.root) return;
    dictEls.filters?.addEventListener('click', onFilterTypeClick);
    dictEls.actionSeg?.addEventListener('click', onSegmentClick);
    dictEls.styleSeg?.addEventListener('click', onSegmentClick);
    dictEls.printBtn?.addEventListener('click', handlePrint);
    window.addEventListener('afterprint', resetPrintable);
    toggleSegmentsForType();
  }

  function setupDictionary() {
    if (!dictEls.root) return;
    dictSets.actions = uniqueSorted(fullAll.map((row) => safeText(row.verb)));
    dictSets.styles = uniqueSorted(fullAll.map((row) => safeText(row.style)));
    if (!dictState.action && dictSets.actions.length) dictState.action = dictSets.actions[0];
    if (!dictState.style && dictSets.styles.length) dictState.style = dictSets.styles[0];
    renderDictSegment(dictEls.actionSeg, dictSets.actions, dictState.action);
    renderDictSegment(dictEls.styleSeg, dictSets.styles, dictState.style);
    updateFilterPills();
    toggleSegmentsForType();
    refreshDictionary(true);
  }

  function onFilterTypeClick(event) {
    const btn = event.target.closest('button[data-type]');
    if (!btn) return;
    const type = btn.dataset.type;
    if (!type) return;
    if (dictState.type === type && type !== 'random') return;
    dictState.type = type;
    if (type !== 'random') dictState.random = null;
    updateFilterPills();
    toggleSegmentsForType();
    refreshDictionary(type === 'random');
  }
  ...
  function onSegmentClick(event) {
    ...
    if (!value) return;
    const segment = event.currentTarget?.dataset?.segment || 'action';
    if (segment === 'action') {
      dictState.action = value;
    } else if (segment === 'style') {
      dictState.style = value;
    }
    if (dictState.type === 'random') {
      dictState.random = { type: segment, value };
    }
    setSegmentActive(event.currentTarget, value);
    refreshDictionary();
  }

  function updateFilterPills() {
    if (!dictEls.filters) return;
    dictEls.filters.querySelectorAll('button[data-type]').forEach((btn) => {
      btn.classList.toggle('is-active', btn.dataset.type === dictState.type);
    });
  }

  function toggleSegmentsForType() {
    if (!dictEls.actionSeg || !dictEls.styleSeg) return;
    if (dictState.type === 'action') {
      dictEls.actionSeg.hidden = false;
      dictEls.styleSeg.hidden = true;
    } else if (dictState.type === 'style') {
      dictEls.actionSeg.hidden = true;
      dictEls.styleSeg.hidden = false;
    } else {
      dictEls.actionSeg.hidden = dictSets.actions.length === 0;
      dictEls.styleSeg.hidden = dictSets.styles.length === 0;
    }
  }

  function renderDictSegment(container, items, selectedValue) {
    if (!container) return;
    container.innerHTML = '';
    if (!items.length) {
      const empty = document.createElement('span');
      empty.className = 'pg-dict__empty';
    ...
    items.forEach((item) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'pg-dict__choice';
      btn.dataset.value = item;
      btn.textContent = item;
      if (item === selectedValue) btn.classList.add('is-active');
  ...
  function refreshDictionary(forceRandom = false) {
    if (!dictEls.root) return;
    if (!fullAll.length) {
      if (dictEls.feedback) dictEls.feedback.textContent = 'Loading prompts...';
      if (dictEls.printBtn) dictEls.printBtn.disabled = true;
      return;
    }
    ...
    if (dictState.type === 'action') {
      if (!dictState.action && dictSets.actions.length) dictState.action = dictSets.actions[0];
      setSegmentActive(dictEls.actionSeg, dictState.action);
      dictEls.actionSeg && (dictEls.actionSeg.hidden = false);
      dictEls.styleSeg && (dictEls.styleSeg.hidden = true);
      if (dictState.action) {
        matches = fullAll.filter((row) => safeText(row.verb) === dictState.action);
        summary = `Action: ${dictState.action}`;
      } else {
        summary = 'No actions available';
      }
      dictState.random = null;
    } else if (dictState.type === 'style') {
      if (!dictState.style && dictSets.styles.length) dictState.style = dictSets.styles[0];
      setSegmentActive(dictEls.styleSeg, dictState.style);
      dictEls.actionSeg && (dictEls.actionSeg.hidden = true);
      dictEls.styleSeg && (dictEls.styleSeg.hidden = false);
      if (dictState.style) {
        matches = fullAll.filter((row) => safeText(row.style) === dictState.style);
        summary = `Style: ${dictState.style}`;
      } else {
        summary = 'No styles available';
      }
      dictState.random = null;
    } else {
      if (!dictState.random || forceRandom) {
        dictState.random = pickRandomSelection();
      }
      const choice = dictState.random;
      if (choice) {
        if (choice.type === 'action') {
          matches = fullAll.filter((row) => safeText(row.verb) === choice.value);
          dictState.action = choice.value;
          setSegmentActive(dictEls.actionSeg, choice.value);
        } else {
          matches = fullAll.filter((row) => safeText(row.style) === choice.value);
          dictState.style = choice.value;
          setSegmentActive(dictEls.styleSeg, choice.value);
        }
        summary = `Random ${choice.type}: ${choice.value}`;
        dictEls.actionSeg && (dictEls.actionSeg.hidden = dictSets.actions.length === 0);
        dictEls.styleSeg && (dictEls.styleSeg.hidden = dictSets.styles.length === 0);
      } else {
        summary = 'No prompts available yet';
        matches = [];
      }
    }

    dictState.matches = matches.slice(0, 10);
    dictState.summary = summary;

    if (dictEls.feedback) {
      const count = matches.length;
      dictEls.feedback.textContent = count
        ? `${summary} - ${count} match${count === 1 ? '' : 'es'}`
        : summary ? `${summary} - no matches` : 'No matches found';
    }
    if (dictEls.printBtn) dictEls.printBtn.disabled = matches.length === 0;
  }

  function pickRandomSelection() {
    const pools = [];
    if (dictSets.actions.length) pools.push({ type: 'action', list: dictSets.actions });
    if (dictSets.styles.length) pools.push({ type: 'style', list: dictSets.styles });
    if (!pools.length) return null;
    const pool = pools[Math.floor(Math.random() * pools.length)];
    const value = pool.list[Math.floor(Math.random() * pool.list.length)];
    return { type: pool.type, value };
  }

  function handlePrint() {
    if (!dictEls.printable || !dictState.matches.length) return;
    const now = new Date().toLocaleString();
    const summary = dictState.summary || 'Prompt Notebook';
    const itemsHTML = dictState.matches.map((item, idx) => formatPrintableItem(item, idx)).join('');
    dictEls.printable.innerHTML = `
      <section class="pg-dict-print__wrap">
        <header class="pg-dict-print__head">
          <h2>Prompt Notebook Pack</h2>
          <p class="pg-dict-print__meta">${escapeHTML(summary)} - ${escapeHTML(now)}</p>
        </header>
        <div class="pg-dict-print__list">${itemsHTML || '<p>No prompts available.</p>'}</div>
      </section>
    `;
    document.body.classList.add('pg-dict-print');
    window.setTimeout(() => {
      window.print();
      window.setTimeout(() => { resetPrintable(); }, 200);
    }, 40);
  }

  function resetPrintable() {
    if (!document.body.classList.contains('pg-dict-print')) return;
    document.body.classList.remove('pg-dict-print');
    if (dictEls.printable) dictEls.printable.innerHTML = '';
  }

  function formatPrintableItem(item, idx) {
    const verb = safeText(item.verb);
    const noun = safeText(item.noun);
    const title = [verb, noun].filter(Boolean).join(' ').trim() || `Prompt ${idx + 1}`;
    const subject = safeText(item.subject);
    const rendered = safeText(item.rendered_prompt || item.prompt || item.instructions || subject);
    const meta = [];
    const format = safeText(item.format);
    const style = safeText(item.style);
    const length = safeText(item.length || item.tokens || item.token_count || item.words);
    const temperature = safeText(item.temperature);
    if (format) meta.push(`Format: ${format}`);
    if (style) meta.push(`Style: ${style}`);
    if (length) meta.push(`Length: ${length}`);
    if (temperature) meta.push(`Temp: ${temperature}`);
    const metaLine = meta.length ? `<div class="pg-dict-print__meta-row">${meta.map(escapeHTML).join(' | ')}</div>` : '';
    const summary = subject ? `<p class="pg-dict-print__summary">${escapeHTML(subject)}</p>` : '';
    return `
      <article class="pg-dict-print__item">
        <h3 class="pg-dict-print__title">${escapeHTML(title)}</h3>
        ${summary}
        ${metaLine}
        <div class="pg-dict-print__body">${escapeHTML(rendered)}</div>
      </article>
    `;
  }
  ...
  function setControlsOpen(open, opts = {}) {
    if (!controlsWrap || !controlsToggle || !controlsPanel) return;
    controlsWrap.dataset.open = open ? 'true' : 'false';
    controlsToggle.setAttribute('aria-expanded', open ? 'true' : 'false');
    controlsPanel.hidden = !open;
    if (open && opts.focusInput && searchEl) {
      requestAnimationFrame(() => searchEl.focus({ preventScroll: true }));
    }
  }

  function isControlsOpen() {
    if (!controlsWrap) return false;
    return controlsWrap.dataset.open === 'true';
  }

  function hasActiveFilters() {
    return !!(searchEl?.value.trim() || (sortEl && sortEl.value !== 'score'));
  }

  function setStatusOpen(open) {
    if (!statusWrap || !statusToggle || !statusEl) return;
    statusWrap.dataset.open = open ? 'true' : 'false';
    statusToggle.setAttribute('aria-expanded', open ? 'true' : 'false');
    if (resultsWrap) resultsWrap.hidden = !open;
    if (statusLabel) statusLabel.textContent = open ? 'Hide prompt matches' : 'Show prompt matches';
  }
  ...
  function attachEvents() {
    ...
    if (statusToggle) {
      statusToggle.addEventListener('click', () => {
        const open = statusWrap?.dataset.open === 'true';
        setStatusOpen(!open);
        if (!open) {
    ...
    if (controlsWrap && controlsToggle && controlsPanel) {
      controlsToggle.addEventListener('click', () => {
        const open = isControlsOpen();
        setControlsOpen(!open, { focusInput: !open });
      });
  ...
  async function init() {
    try {
      ...
      if (dictEls.feedback) dictEls.feedback.textContent = 'Could not load prompts.';
      if (dictEls.printBtn) dictEls.printBtn.disabled = true;
      renderDictSegment(dictEls.actionSeg, [], '');
      renderDictSegment(dictEls.styleSeg, [], '');
      toggleSegmentsForType();
    }
  }
